...
export default function Hero() {
	return (
		...
		>
			<div
				className="absolute inset-0 bg-[linear-gradient(180deg,rgba(0,0,0,0.82),rgba(0,0,0,0.6),rgba(0,0,0,0.85))]"
				aria-hidden
			/>
			...
			<div className="section-inner relative flex flex-col items-center gap-10 text-center lg:max-w-4xl">
				<div className="section-heading mb-0 space-y-6">
					<div className="section-eyebrow border border-white/20 bg-white/10 text-[#FACC14]">
						<span
							className="inline-flex h-2 w-2 rounded-full bg-[#FACC14]"
							aria-hidden
						/>
						24/7 live dispatch
					</div>
					<h1 className="text-white text-5xl md:text-6xl lg:text-5xl font-extrabold drop-shadow-2xl">
						Emergency vehicle recovery, towing & roadside rescue — whenever you
						need it
					</h1>

					<p className="section-description text-white">
						From motorway breakdowns to driveway transport, our control room keeps
						Greater Manchester moving with real-time dispatch, specialist flatbeds,
						and pricing confirmed before we roll.
					</p>
				</div>
				...
				<div className="grid w-full max-w-4xl gap-4 sm:grid-cols-3">
					{stats.map(({ value, label }) => (
						<div
							key={label}
							className="rounded-2xl border border-white/20 bg-white/10 px-6 py-6 text-center backdrop-blur-sm"
						>
